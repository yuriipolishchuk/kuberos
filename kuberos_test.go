...
func TestAuthCodeURL(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			url: "https://auth.example.org?access_type=offline&client_id=testClientID&prompt=consent&redirect_uri=http%3A%2F%2Fexample.com%2Fui&response_type=code&scope=openid+profile+email+groups&state=state",
		},
		{
			name: "CustomScopes",
			c: &oauth2.Config{
